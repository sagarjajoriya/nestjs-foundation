-- CreateEnum
CREATE TYPE "oauth_provider" AS ENUM ('GOOGLE', 'GITHUB', 'MICROSOFT', 'DISCORD', 'APPLE');
...
-- Partial unique index: email must be unique among ACTIVE (non-soft-deleted)
-- users, allowing a soft-deleted user's email to be re-registered. This cannot
-- be expressed in schema.prisma (Prisma has no partial-index syntax), so it is
-- managed here. See the User model note in schema.prisma.
